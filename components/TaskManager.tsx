...
export default function TaskManager({ activeTab, tasks, setTasks, teamMembers, currentUser, savedReports = [] }: TaskManagerProps) {
  ...
  const stats = useMemo(() => {
    ...
    const staffPerformance: Record<string, { name: string, total: number, completed: number, points: number }> = {};
    
    tasks.forEach(t => {
        if(t.assignedToId) {
            const member = teamMembers.find(m => m.id === t.assignedToId);
            const name = member ? member.name : 'Unknown';
            if(!staffPerformance[name]) staffPerformance[name] = { name, total: 0, completed: 0, points: 0 };
    ...
    const leaderboard = Object.values(staffPerformance).sort((a,b) => b.points - a.points);
    ...
    return (
      <div className="space-y-6 pb-20">
         <div className="bg-indigo-600 rounded-2xl p-8 text-white shadow-xl shadow-indigo-200">
            ...
            <p className="text-indigo-100">Overview of all operational tasks and team progress.</p>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
               <div>
                  ...
                  <h3 className="text-3xl font-black text-emerald-500">{stats.completed}</h3>
               </div>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <h3 className="font-bold text-slate-800 mb-4">Top Performers (Tasks Done)</h3>
                <div className="space-y-4">
                    {stats.leaderboard.slice(0, 5).map((staff, idx) => (
                        <div key={idx} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                            <div className="flex items-center gap-3">
                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm ${idx === 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-white border border-slate-200 text-slate-500'}`}>
                                    {idx + 1}
                                </div>
                                <span className="font-bold text-slate-700">{staff.name}</span>
                            </div>
                            <span className="font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded text-xs">{staff.completed} Done</span>
                        </div>
                    ))}
                    {stats.leaderboard.length === 0 && <p className="text-slate-400 text-center py-4">No data yet.</p>}
                </div>
            </div>
         </div>
      </div>
                  ...
                  <div className="overflow-x-auto h-full">
                      <table className="w-full text-left text-sm">
                          <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
                              <tr>
                                  <th className="px-6 py-4 font-bold text-slate-600">Task Title</th>
                          ...
                          <tbody className="divide-y divide-slate-100">
                              {filteredTasks.map(task => {
                                  ...
                                  return (
                                      <tr key={task.id} className="hover:bg-slate-50 group">
                                          ...
                                          <td className="px-6 py-4">
                                              <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs uppercase font-bold tracking-wide">{task.type}</span>
                                          </td>
                                          <td className="px-6 py-4">
                              ...
                              {filteredTasks.length === 0 && (
                                  <tr>
                                      <td colSpan={5} className="px-6 py-12 text-center text-slate-400">No tasks found.</td>
                                  </tr>
                              )}
                          </tbody>
                      </table>
          ...
          <div className="h-full flex flex-col space-y-6">
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm shrink-0">
                  <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                      <Star className="text-yellow-500 fill-current" /> Staff Performance Ratings
                  </h2>
                  <p className="text-slate-500 text-sm">Automated scoring based on task completion and efficiency.</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
                  {stats.leaderboard.map((staff, idx) => {
                      ...
                      return (
                          <div key={staff.name} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
                              {/* Rank Badge */}
                              <div className={`absolute top-0 right-0 p-3 rounded-bl-2xl font-bold text-lg w-12 h-12 flex items-center justify-center text-white ${
                                  idx === 0 ? 'bg-yellow-400' : idx === 1 ? 'bg-slate-400' : idx === 2 ? 'bg-amber-700' : 'bg-slate-100 text-slate-400'
                              }`}>
                                  {idx === 0 ? <Award size={20} className="fill-current"/> : `#${idx + 1}`}
                              </div>
                              ...
                              <div className="flex items-center gap-4 mb-6">
                                  ...
                                  <div>
                                      ...
                                      <div className="flex items-center gap-1 text-yellow-500">
                                          {[1,2,3,4,5].map(star => (
                                              <Star key={star} size={14} className={efficiency >= star * 20 ? 'fill-current' : 'text-slate-200'} />
                                          ))}
                                      </div>
                                  </div>
                              </div>
                              ...
                              <div className="grid grid-cols-2 gap-4 mb-4">
                                  <div className="bg-slate-50 p-3 rounded-xl text-center">
                                      <span className="block text-xs font-bold text-slate-400 uppercase">Tasks</span>
                                      <span className="block text-xl font-black text-slate-700">{staff.total}</span>
                                  </div>
                                  <div className="bg-emerald-50 p-3 rounded-xl text-center">
                                      <span className="block text-xs font-bold text-emerald-600 uppercase">Score</span>
                                      <span className="block text-xl font-black text-emerald-600">{staff.points}</span>
                                  </div>
                              </div>
                              ...
                              <div className="space-y-1">
                                  ...
                                  <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                                      <div className="bg-indigo-500 h-full rounded-full transition-all duration-1000" style={{ width: `${efficiency}%` }}></div>
                                  </div>
                              </div>
                          </div>
                      );
